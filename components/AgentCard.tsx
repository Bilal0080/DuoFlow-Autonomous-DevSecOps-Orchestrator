...
const AgentCard: React.FC<AgentCardProps> = ({ agent, status }) => {
  ...
  // Trigger a brief animation when status changes
  useEffect(() => {
    setIsUpdating(true);
    ...
    >
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-800/50">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[10px] font-bold uppercase tracking-wider transition-colors duration-500 ${status === AgentStatus.IDLE ? 'text-slate-500' : 'text-slate-200'}`}>
            {getStatusText()}
          </span>
        </div>
        
        {/* Activity pulse for "ACTING" state */}
